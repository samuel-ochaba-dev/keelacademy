...
the two ever disagree, fix the linter and this page together.
...
`content/tools/check-unit-consistency.py <unit>` must pass: same headings, same numbers, same document names, same banned words, same word budget everywhere; every FAQ anchor exists; no number taught by only one file.
